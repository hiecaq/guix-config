...
#+begin_src emacs-lisp
  ...
  (setup consult
    (consult-info-define "emacs" "efaq" "elisp" "eintr" "cl" "compat" "eshell")
...
For example, if the cursor is at the symbol =button=, the default behavior selects =face=, =library= and =file=, except for =symbol=.  This for example forbid I to highlight the word =button= with =H=.  I find this behavior super confusing, so let's fix it:
#+begin_src emacs-lisp
