...
- [[#emacs][Emacs]]
  ...
  - [[#early-initialization][Early Initialization]]
    - [[#android-exec-path][Android Exec Path]]
    - [[#packages-1][Packages]]
  ...
  - [[#main-configurations][Main Configurations]]
    ...
    - [[#transient][Transient]]
    - [[#evil][Evil]]
      - [[#split-windows-in-emacs-way][Split windows in Emacs way]]
    ...
    - [[#consult][Consult]]
    - [[#embark][Embark]]
      - [[#fix-symbol-target-in-elisp][Fix symbol target in Elisp]]
      ...
      - [[#preview-for-non-consult-commands-using-embark][Preview for non-Consult commands using Embark]]
    - [[#tempel][Tempel]]
...
#+begin_src scheme :tangle "build/system-configuration.scm" :noweb yes
  ...
  (use-package-modules bootloaders fonts games android virtualization firmware)
...
*** QEMU
See the cookbook for details about setting up the bridge.
...
#+begin_src scheme
  (extra-special-file "/etc/qemu/bridge.conf"
                      (plain-file "qemu-bridge.conf" "allow br0\n"))
#+end_src
...
*** GDM
 here is how to do that in Guix's way to add configurations to =/etc/X11/xorg.conf.d/=.  
#+begin_src scheme
  (gdm-service-type
...
**** Fix cursor
Somehow after log-in my mouse cursor disappears.  To fix it, apply the fix from https://forums.linuxmint.com/viewtopic.php?t=424779:
#+begin_src scheme :noweb-ref system-helper
  (define %fix-amd-disappearing-cursor-conf "
    Section \"OutputClass\"
      Identifier \"AMD fix cursor\"
      MatchDriver \"amdgpu\"
      Driver \"amdgpu\"
...
**** Fix screen tearing
See https://wiki.archlinux.org/title/AMDGPU for details.
#+begin_src scheme :noweb-ref system-helper
  (define %fix-amd-screen-tearing "
  Section \"OutputClass\"
    Identifier \"AMD fix screen tearing\"
    MatchDriver \"amdgpu\"
    Driver \"amdgpu\"
...
** MIME
This defines how file association works.

To use MIME associations, we need tools like =xdg-mime= and =xdg-open=, which are provided by:
#+begin_src scheme :noweb-ref xdg-service
  (simple-service
   'home-xdg-utils
   home-profile-service-type
...
* Securities
This includes encryption, signing, password management, etc.
...
Load audio files with the exact same names:
#+begin_src conf
...
#+begin_src conf
  ...
  Ctrl+] add sub-delay       +0.1
  Ctrl+{ add secondary-sub-delay -0.1
  Ctrl+} add secondary-sub-delay +0.1
#+end_src
...
This is an attempt to use containerized firefox.  XDG Desktop Portal does not work correctly with it, so I've temporarily switched to using librewolf on the host directly.  It should still work if you don't need features provided by XDG Desktop Portal, so I keep the code here for reference.
...
#+begin_src scheme :tangle "build/hiecaq/packages/emacs-xyz.scm" :noweb-ref nil
  (define-module (hiecaq packages emacs-xyz)
    ...
    #:use-module ((gnu packages rust-apps) #:select (difftastic))
...
*** Android Exec Path
See [[https://ftp.gnu.org/gnu/emacs/android/README][Android README]].
    ...
    (with-selected-frame frame
      (when (display-graphic-p)
        (let* ((families (font-family-list))
          ...
          (when (memq system-type '(gnu/linux android))
...
stolen from [[https://howardism.org/Technical/Emacs/eshell-why.html][Howard Abrams]], we can define a wrapper for PCRE-flavored rx.  Notably this is usable in eshell!
#+begin_src emacs-lisp
  (setup pcre2el
...
I find that =-= is more consistent for going "up" than =u= in Evil:
#+begin_src emacs-lisp
  (setup info
...
*** Display time
=display-time-mode= is a built-in mode that show the current time (in real life) among other misc things inside ~global-mode-string~.
#+begin_src emacs-lisp
  (setup time
    (:option display-time-24hr-format t
             display-time-default-load-average nil)
    (:with-feature display-time
      (:enable)))
#+end_src

*** Display battery
Android is capable of displaying battery status:
#+begin_src emacs-lisp
  (setup display-battery
    (:system 'android)
    (:enable))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defvar-local my-mode-line-coding-system-indicator
      '(:eval
        (unless (memq (coding-system-type buffer-file-coding-system)
                      '(undecided utf-8))
          (concat (symbol-name (coding-system-base buffer-file-coding-system)) " "))))

  (put 'my-mode-line-coding-system-indicator 'risky-local-variable t)

  (defvar-local my-mode-line-eol-indicator
      '(:eval
        (unless (eq (coding-system-eol-type buffer-file-coding-system) 0)
          (concat (coding-system-eol-type-mnemonic buffer-file-coding-system) " "))))

  (put 'my-mode-line-eol-indicator 'risky-local-variable t)

  (defvar-local my-mode-line-indicators (list my-mode-line-recursion-indicator
                                              my-mode-line-coding-system-indicator
                                              my-mode-line-eol-indicator
                                              ...
                                              '(:eval (pcase (window-dedicated-p) ('t "DEDI ") ('nil nil) (_ "Dedi ")))
...
*** Tab bar
The built-in =tab-bar-mode= is similar to how tab works in Vim: each tab is a standalone multi-window layout, similar to frames.  There is also a built-in =tab-line-mode= which enables one tab per window ("window" in Emacs' terminology) workflow.

Tab-bar format is also constomizable, similar to ~mode-line-format~.  When it contains ~tab-bar-format-global~, ~global-mode-string~ will no longer be showed in  ~mode-line-misc-info~, but in ~tab-bar-format-global~ instead.
#+begin_src emacs-lisp
  (defvar-local my-tab-bar-format nil
    "My `tab-bar-format', for easy toggle between the default version.")

  (defun my-toggle-tab-bar-format ()
    (interactive)
    (let* ((standard (eval (car (get 'tab-bar-format 'standard-value))))
           (new-format (if (eq standard (default-value 'tab-bar-format))
                           my-tab-bar-format
                         standard)))
      (setq-default tab-bar-format new-format)
      (kill-local-variable 'tab-bar-format)
      (force-tab-bar-update)))

  (setopt my-tab-bar-format '(tab-bar-format-history
                              tab-bar-format-tabs
                              tab-bar-separator
  ...
  (setup tab-bar
    (:option tab-bar-format my-tab-bar-format
...
#+begin_src scheme :tangle "build/hiecaq/packages/emacs-xyz.scm" :noweb-ref nil
  (define-public emacs-evil-collection-next
    (let ((commit "00e71118ab43d8eebf619c8ac00d165b21b9c1e7")
      ...
      (package
       ...
       (source
        (origin
         ...
         (sha256
          (base32
           "16yrp140hnij647jnqxi73ny7y78qnasf2ihgdr6bf43vpzkzw13"))))
       (propagated-inputs
...
=g8= in Vim and Evil displays the info of the character under cursor (I think that =8= stands for UTF-8).  We can do better by displaying the line number too, which diminishing the usefulness of =display-line-numbers-mode=.  Line number information is not very useful for structural editing so we don't have to display it all the time.
#+begin_src emacs-lisp
  (setup simple
    (defun my-what-cursor-position (&optional detail)
      (interactive "P")
      (let ((cursor-position (let ((inhibit-message t)
                                   (message-log-max nil))
                               (what-cursor-position detail)))
            (line-position (let ((inhibit-message t)
                                 (message-log-max nil))
                             (what-line))))
        (message "%s, %s" cursor-position line-position)))
    (:global
     (:with-state (normal)
       (:bind "g !" #'my-what-cursor-position))))
#+end_src

**** Split windows in Emacs way
Evil by default follows Vim's behaviors when splitting windows, which is opposite to Emacs'.  However, other Emacs packages usually follow the Emacs' way, so for better consistency Evil should do it too.
#+begin_src emacs-lisp
  (setup evil
    (:option evil-split-window-below t
...
*** WGrep
[[https://github.com/mhayashi1120/Emacs-wgrep][wgrep]] enables editing grep buffers, and the updates will be applied to the actual buffers/files. (especially useful for =embark= exports of ~consult-grep~ families).
#+begin_src scheme
  (simple-service
   'home-emacs-wgrep
   home-emacs-service-type
...
Orderless needs [[https://github.com/minad/consult/wiki#orderless-style-dispatchers-ensure-that-the--regexp-works-with-consult-buffer][some hack]] to work with ~consult-buffer~ and friends. Stolen the basics from [[https://github.com/minad/consult/wiki#minads-orderless-configuration][minad's]].  It breaks =$= in =consult-grep= family, so I fix it with a dynamic-scoped variable.
#+begin_src emacs-lisp
  (setup (:require orderless)
    ...
    (defvar +orderless-consult-dispatch-enabled t
      "Whether enable `+orderless-consult-dispatch' hack or not.")
    ...
    (defun +orderless-consult-dispatch (word _index _total)
      (cond
       ((not +orderless-consult-dispatch-enabled) nil)
...
#+begin_src emacs-lisp
  (setup orderless
    (:option orderless-style-dispatchers '(orderless-kwd-dispatch
...
#+begin_src emacs-lisp
  (setup orderless
    (:when-loaded
      (:option (prepend* orderless-affix-dispatch-alist)
               `((?' . ,#'my-orderless-rx)
                 (?- . ,#'orderless-prefixes)
                 (?_ . ,#'orderless-regexp)))))
#+end_src
...
For ~consult-grep~ families and ~consult-find~ families, it is possible to convert orderless patterns into their PCRE pattern inputs, as suggested by the [[https://github.com/minad/consult/wiki#use-orderless-as-pattern-compiler-for-consult-grepripgrepfind][Wiki]].  I modified it to disable ~+orderless-consult-dispatch~.
#+begin_src emacs-lisp
  (setup consult
    (defun consult--orderless-regexp-compiler (input type &rest _config)
      (setq input
            (cdr (let ((+orderless-consult-dispatch-enabled nil))
                   (orderless-compile input))))
      (cons
...
#+begin_src emacs-lisp
  ...
  (setup consult
    (consult-info-define "emacs" "efaq" "elisp" "eintr" "cl" "eshell")
    (consult-info-define "org" "orgguide" "org-roam" "org-super-agenda")
    (consult-info-define 'completion "vertico" "consult" "marginalia" "orderless" "embark" "corfu" "tempel")
    (consult-info-define "guix" "guix-cookbook" "emacs-guix" "guile"))
#+end_src
...
**** Fix symbol target in Elisp
The default behavior of embark in elisp buffers is quite confusing: if the symbol has the same name as anything special (a library, a package, etc), the target at point will be those instead.  That is, the symbol target is not produced.

For example, if the cursor is at the symbol =button=, the default behavior selects =face=, =library= and =file=, except for =symbol=.  This for example forbids I to highlight the word =button= with =H=.  I find this behavior super confusing, so let's fix it:
#+begin_src emacs-lisp
  (define-advice embark--identifier-types (:filter-return (ret) fix-elisp-symbol)
    "Produce symbol target in `emacs-lisp-mode' when appropriate."
    (if (or (memq 'symbol ret)
            (memq 'identifier ret))
        ret
      (cons 'symbol ret)))
#+end_src
      ...
      (pcase-let ((`(,targets ,k) args))
        (list targets
              (if-let (((cdr targets)) ;; len >= 2
                       ...
                       ((or (and prefix-arg (listp prefix-arg))
...
#+begin_src emacs-lisp
  (setup embark
    (defun embark-target-buffer-current ()
      (when-let ((buffer (buffer-name)))
        (cons 'buffer-current buffer)))

    (defvar-keymap buffer-current-map
    ...
    (:when-loaded
      (:option (prepend my-embark-leader-target-finder) #'embark-target-buffer-current
               (prepend embark-keymap-alist) '(buffer-current . buffer-current-map))))
#+end_src

#+begin_src emacs-lisp
  (setup embark
    (defun embark-target-file-current ()
      (when-let ((file (buffer-file-name)))
        (cons 'file-current file)))

    (defvar-keymap file-current-map
      :doc "Commands to act on current file."
      :parent embark-file-map
      "g" #'revert-buffer)

    (:when-loaded
      (:option (prepend my-embark-leader-target-finder) #'embark-target-file-current
               (prepend embark-keymap-alist) '(file-current . file-current-map))))
#+end_src


#+begin_src emacs-lisp
  (setup embark
    (defun embark-target-tab-current ()
      (when-let* ((tab-bar-mode)
                  (fboundp #'tab-bar--current-tab-find)
                  (current-tab (tab-bar--current-tab-find))
                  (tab (cdr current-tab))
                  (tab-name (alist-get 'name tab)))
        (cons 'tab-current tab-name)))

    (:when-loaded
      (:option (prepend my-embark-leader-target-finder) #'embark-target-tab-current
               (prepend embark-keymap-alist) '(tab-current . tab-prefix-map))
...
**** Preview for non-Consult commands using Embark
Stolen from the [[https://github.com/minad/consult/wiki#manual-preview-for-non-consult-commands-using-embark][wiki]]
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (setup tempel
    (define-advice tempel--element (:before-until (region elt) save-region)
      (pcase elt
        (`(rs ,name)
         (when (and (not (alist-get name (cdar tempel--active)))
                    region)
           (setf (alist-get name (cdar tempel--active))
...
*** Compile
Compilation mode is the mode used in =M-x compile= and =grep=.
#+begin_src emacs-lisp
  (setup compile
    (defun my-compilation-finish-notification (buffer string)
      (notifications-notify
       :title (buffer-name buffer)
       :body string
       :urgency (if (get-text-property 0 'face (car mode-line-process))
                    'critical
                  'normal)))
    (:with-hook compilation-finish-functions
      (:hook my-compilation-finish-notification)))
#+end_src
...
Evil's eshell integration, provided by =evil-collection=, introduces some operators that should be known by =evil-surround=:
#+begin_src emacs-lisp
  (setup evil-surround
    (:when-loaded
      (:option (prepend* evil-surround-operator-alist)
               '((evil-collection-eshell-evil-change . change)
                 (evil-collection-eshell-evil-change-line . change)
                 (evil-collection-eshell-evil-delete . delete)
                 (evil-collection-eshell-evil-delete-line . delete)))))
#+end_src
...
**** Notifications
Eshell can also notify us when a command is finished.  The idea is from [[https://old.reddit.com/r/emacs/comments/1owx72v/eshell_automatic_notification_when_command/][this reddit thread]].
#+begin_src emacs-lisp
  (setup eshell
    (defvar-local my-eshell-notification-command-start-time nil
      "The time when the command starts.")

    (defun my-notification-time-significant-p (start-time)
      "Whether the time elapsed is significant."
      (let ((second (time-convert (time-since start-time) 'integer)))
        (> second 3)))

    (defun eshell-pre-command-hook@set-time ()
      (setq my-eshell-notification-command-start-time (current-time)))

    (defun eshell-post-command-hook@notify ()
      (when-let* ((eshell (current-buffer))
                  (selected (window-buffer (selected-window)))
                  ((not (eq eshell selected)))
                  (start-time (or my-eshell-notification-command-start-time 0))
                  ((my-notification-time-significant-p start-time)))
        (notifications-notify
         :title (buffer-name eshell)
         :body (format "done with %d" eshell-last-command-status)
         :urgency (if (eshell-exit-success-p)
                      'normal
                    'critical))))

    (:local-hook eshell-pre-command-hook eshell-pre-command-hook@set-time)
    (:local-hook eshell-post-command-hook eshell-post-command-hook@notify))
#+end_src

This makes the built-in mode-line indicator no longer needed.
#+begin_src emacs-lisp
  (setup eshell
    (:option eshell-status-in-mode-line nil))
#+end_src
*** Ediff
#+begin_src emacs-lisp
  (setup ediff
...
#+begin_src scheme :tangle "build/hiecaq/packages/emacs-xyz.scm" :noweb-ref nil
  (define-public emacs-org-roam-next
    (let ((commit "e234c180952a13497cd59325d23f0032e2cf9dc2")
          (revision "1"))
      (package
        ...
        (source
         (origin
           ...
           (file-name (git-file-name name version))
           (sha256
            (base32 "1kgpg96hx5wfjw22qxxk37c00szqb5f3zfzzrq0bb3h6d4z7lvv7"))))
        (propagated-inputs
...
#+begin_src scheme :tangle "build/hiecaq/packages/emacs-xyz.scm" :noweb-ref nil
  (define-public emacs-org-roam-ql
    (let ((commit "b15688452aab644bb57fb12ca9e940cbf2f067d9")
          (last-release-version "0.3")
          (revision "1")
      ...
      (package
       ...
       (source
        (origin
         ...
         (sha256
          (base32
           "04l7b88ii66p9k62088n1465v4cqwlpq7fng990z9vkvhlkp07is"))))
       (build-system emacs-build-system)
       (propagated-inputs
...
#+begin_src emacs-lisp
  (setup org-roam
    (defun consult--org-roam-buffer-pair (buffer)
      "Return a pair of title of BUFFER and BUFFER."
      (cons (with-current-buffer buffer
              (or (org-roam-node-file-title
                   (my-org-roam-node-this-file 'assert))
                  (buffer-name buffer)))
            buffer))
...
***** Easy Draw
[[https://github.com/misohena/el-easydraw][easydraw]] allows you to draw SVG images directly inside Emacs.  It even has clickable toolbars and a color picker.  All features are implemented only with built-in.  Very insane and impressive and every Emacs user should try this package!
#+begin_src scheme :tangle "build/hiecaq/packages/emacs-xyz.scm" :noweb-ref nil
  (define-public emacs-easydraw
    (let ((commit "8007f50c1c1734325c47939904f486753c7dd8ee")
          (last-release-version "1.2.1")
          (revision "0")
      ...
      (package
        ...
        (source
         (origin
           ...
           (sha256
            (base32
             "1q0cx5h5dc2x1drqlfvhpw178dkxmw3z247141g2sj0jx2bsji30"))))
        (build-system emacs-build-system)
        (arguments
         (list
          #:tests? #false
          #:include #~(cons "^msg/" %default-include)
          #:phases
          #~(modify-phases %standard-phases
              (add-after 'unpack 'delete-screenshots
                (lambda _
                  (delete-file-recursively "screenshot"))))))
        (home-page url)
        (synopsis "Embedded drawing tool for Emacs")
        (description "This package is a drawing tool that runs inside Emacs.")
        (license license:gpl3+))))
#+end_src
...
Integrate it with =embark=:
#+begin_src emacs-lisp
  (setup embark
    (defun embark-target-eglot-symbol-at-point ()
      (when-let* ((face (get-char-property (point) 'face))
                  ((eq face 'eglot-highlight-symbol-face))
                  (bounds (bounds-of-thing-at-point 'symbol))
                  (name (buffer-substring (car bounds) (cdr bounds))))
        `(eglot-symbol ,name . ,bounds)))

    (defvar-keymap embark-eglot-symbol-map
      :doc "Commands to act on current eglot-awared symbol."
    ...
    (:when-loaded
      (:option (prepend embark-target-finders) #'embark-target-eglot-symbol-at-point
               (prepend embark-keymap-alist) '(eglot-symbol . embark-eglot-symbol-map))
      (mapc
       (lambda (cmd)
         (cl-pushnew 'embark--ignore-target
                     (alist-get cmd embark-target-injection-hooks)))
...
#+begin_src emacs-lisp
  (setup rust-mode
    ...
    (:option rust-rustfmt-switches '("--edition" "2024")))
#+end_src
  ...
  (setup browse-url
    ...
    (:when-loaded
      (:option (prepend browse-url-handlers)
               `(,(rx-let ((suffix (seq "." (or "mkv" "mp4" "webm"))))
                    (rx (or (seq suffix eos)
                            (seq
                             bos
                             (or (seq "https://" (or "www" "m" "live") ".bilibili.com/video/")
                                 "https://youtu.be/"
                                 (seq "https://" (or "www" "m") ".youtube.com/" (or "watch" "embed"))
                                 (seq "https://cdn.discordapp.com/attachments/" (+ (not "?")) suffix))))))
                 . browse-url-mpv))))
...
When the input is partially decided, PYIM use a hack to determine the unused input left (~pyim-liberime--get-code~), but it is triggered based on a fixed set of schemata, which only includes RIME's default ones.  Since schemata I use are all quanpin-like, I force it to take the corresponding branch:
#+begin_src emacs-lisp
...
For =eat-mode= (i.e. the virtual terminal), its shell integration pushes command histories to ~shell-command-history~.  We can tell =consult= to utilize it:
#+begin_src emacs-lisp
  (setup consult
    (:when-loaded
      (:option (prepend consult-mode-histories)
               '(eat-mode shell-command-history nil beginning-of-line))))

  (setup eat
    (:rebind #'eat-line-history-isearch-backward-regexp #'consult-history))
#+end_src

Directory tracking seems to be broken if the path contains CJK characters.  We can turn it off:
#+begin_src emacs-lisp
  (setup eat
    (:option eat-enable-directory-tracking nil))
#+end_src
...
*** OAUTH2
[[https://elpa.gnu.org/packages/oauth2.html][oauth2]] implements an OAUTH2 client in Emacs.
#+begin_src scheme
  (simple-service
   'home-emacs-oauth2
   home-emacs-service-type
...
Under the hood, it uses =plstore=, which is a library providing (optionally encrypted) persistent plist stores.  It uses =epa= and =epg=, which then talk to GPG.  By setting ~plstore-encrypt-to~ it'll silently use the GPG key matches the given string.
#+begin_src emacs-lisp
  (setup plstore
    (:option plstore-encrypt-to "hiecaq"))
#+end_src

BTW, to test if GPG/ =epg= / =epa= is working as intended,
#+begin_src elisp :exports none
  (epg-list-keys (epg-make-context 'OpenPGP) `(,plstore-encrypt-to))
#+end_src

No littering!
#+begin_src emacs-lisp
  (setup oauth2
    (:option oauth2-token-file (exdg-state "oauth2.plstore")))
#+end_src

It supports PCKE but unfortunately only with S256 method at time of writing.   However, the websites I use require the plain method:
#+begin_src emacs-lisp
  (setup oauth2
    (define-advice oauth2--build-url (:filter-args (args) use-pkce-plain)
      "Modify `code_challenge_method' to be plain if presented."
      (let ((address (car args))
            (data (cdr args)))
        `(,address ,@(if (plist-get data "code_challenge_method" #'equal)
                         (plist-put data "code_challenge_method" "plain" #'equal)
                       data))))

    (define-advice oauth2--get-challenge-from-verifier (:override (code-verifier) use-pkce-plain)
      "Return code-verifier as-is."
      code-verifier))
#+end_src
