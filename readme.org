...
I found that very often I want the buffer opened by embark to be somewhere I assign. Adapted from [[https://karthinks.com/software/fifteen-ways-to-use-embark/][Karthik Chikmagalur's hack]], I now have a way of picking the window (or splitting on-the-fly) by calling ~my-embark-act-other-window~.
#+begin_src emacs-lisp
  (setup embark
  ...
  themselves, which should be fixed in other way later.")
    ...
    (:with-map embark-general-map
      (:bind "O" #'my-embark-act-other-window)))
#+end_src
