...
I found that very often I want the buffer opened by embark to be somewhere I assign. Adapted from [[https://karthinks.com/software/fifteen-ways-to-use-embark/][Karthik Chikmagalur's hack]], I now have a way of picking the window (or splitting on-the-fly) by calling ~my-embark-act-other-window~. For minibuffer things are a little bit complicated, and currently I'm using a toggle outside of Embark directly.
#+begin_src emacs-lisp
  (setup embark
    (defvar my-embark--action-ace-window-p nil
      "Whether call `ace-window' before the action.")

    (defvar my-embark--action-ace-window-blacklist (list
                                                    #'embark-done
                                                    #'embark-cycle
                                                    #'embark-toggle-quit
                                                    #'describe-symbol
                                                    #'apropos)
      "Never call `ace-window' for these actions. These are the actions that pop windows
  themselves, which should be fixed in other way later.")

    (cl-defun my-embark--ace-window (&rest rest &key run action quit &allow-other-keys)
      "call `ace-window' before the action."
      (when (and my-embark--action-ace-window-p
                 quit
                 (not (embark--action-repeatable-p action))
                 (not (memq action my-embark--action-ace-window-blacklist)))
        (setq my-embark--action-ace-window-p nil)
        (with-demoted-errors "%s"
          (require 'ace-window)
          (let ((aw-dispatch-always t))
            (aw-switch-to-window (aw-select nil)))))
      (apply run :action action :quit quit rest))

    (cl-pushnew 'my-embark--ace-window
                (alist-get :always embark-around-action-hooks))

    (defun my-embark-act-other-window (&optional arg)
      "Perform the following action in ace window."
      (interactive "P")
      (let ((my-embark--action-ace-window-p t))
        (embark-act arg)))

    (defun my-toggle-embark-ace-window ()
      (interactive)
      (when (minibufferp)
        (setq my-embark--action-ace-window-p (not my-embark--action-ace-window-p))
        (message "ace-window-p: [%s]" my-embark--action-ace-window-p)))

    (:with-map vertico-map
      (:bind "C-o" #'my-toggle-embark-ace-window)))
#+end_src
