...
#+last_modified: [2024-11-26 Tue 23:35]
#+startup: indent
...
*** Visual Fill Column
[[https://codeberg.org/joostkremers/visual-fill-column][this package]] is quite useful for the wide screen nowadays: when using ~visual-line-mode~, it wraps the line at the ~fill-column~ (which is usually 80) instead of at the right margin of the window.
#+begin_src scheme
  (simple-service
   'home-emacs-visual-fill-column
   home-emacs-service-type
   (home-emacs-extension
    (packages
     (list
...
#+begin_src emacs-lisp :noweb yes
  (setup org
    <<org-setup>>
    (:hook visual-line-fill-column-mode variable-pitch-mode))
#+end_src
...
I dont' use ~dired-dwim-target~ anymore, because using "future history" is convenient enough.
#+begin_src emacs-lisp
  (setup dired
    (:option dired-dwim-target nil))
#+end_src
